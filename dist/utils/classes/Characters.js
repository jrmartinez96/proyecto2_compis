...
exports.CharacterSetDecl = exports.Char = void 0;
class Char {
...
class BasicSet {
    constructor() {
        this.value = '';
        this.type = -1; // 0: string, 1: ident, 2: char, 3: char to char
        this.addition = null; // si es null entonces no hay signo
    ...
    toRegularExpression(currentCharactersDecl) {
...
class CharacterSet {
    ...
    newSetCharacterSet(set) {
        ...
        if (set[0] === "+" || set[0] === "-") {
            ...
            if (plusSignIndex !== -1)
                plusSignIndex = plusSignIndex + 1;
            if (minusSignIndex !== -1)
                minusSignIndex = minusSignIndex + 1;
        }
        if (plusSignIndex === -1)
            plusSignIndex = 1000;
        if (minusSignIndex === -1)
    ...
    differenceBetweenBasicSets(basicSet1, basicSet2, currentCharactersDecl) {
        let regularExpression = '';
    ...
    deleteAllOccurences(character, base) {
        let newBase = base;
...
exports.CharacterSetDecl = CharacterSetDecl;
